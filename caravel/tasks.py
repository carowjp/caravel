...
def get_sql_results_async(engine, query, orm_session):
    ...
    if query.select_as_cta_used:
        ...
        try:
            ...
            cursor = engine.execute(
                query.select_sql, schema=query.schema).cursor
            while cursor.poll():
    ...
    response = fetch_response_from_cursor(cursor, query)
...
def get_sql_results_sync(engine, query):
    ...
    try:
        ...
        return fail_query(query, utils.error_msg_from_exception(e))
    response = fetch_response_from_cursor(result_proxy.cursor, query)
    query.status = models.QueryStatus.FINISHED
    return response
...
def fetch_response_from_cursor(cursor, query):
